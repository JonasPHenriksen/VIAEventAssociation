...
public class InvitationSeedFactory
{
  ...
  {
    var tmpInvitations = JsonSerializer.Deserialize<List<TmpInvitation>>(InvitationsAsJson);
    ...
    private static void AddInvitationsToEvent(VeadatabaseProductionContext context, string EventId,
        List<TmpInvitation> tmpInvitations)
    {
        IEnumerable<Invitation> invitations = tmpInvitations
            .Where(i => i.EventId == EventId)
            .Select(invt => new Invitation
            {
                GuestId = invt.GuestId,
                VeaEventId = invt.EventId,
                Status = invt.Status
            });
        context.Invitations.AddRange(invitations);
    }
    
    public record TmpInvitation(string GuestId, string EventId, string Status);
