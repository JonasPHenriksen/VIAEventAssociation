...
public class ParticipationSeedFactory
{
  ...
  {
    var tmpInvitations = JsonSerializer.Deserialize<List<TmpParticipants>>(ParticipantsAsJson);
    var events = context.Events.ToList();

    foreach (var e in events)
    {
      AddInvitationsToEvent(context, e.EventId, tmpInvitations);
    }
  }
  
  private static void AddInvitationsToEvent(VeadatabaseProductionContext context, string EventId,
    List<TmpParticipants> tmpInvitations)
  {
    IEnumerable<GuestId> invitations = tmpInvitations
      .Where(i => i.EventId == EventId)
      .Select(invt => new GuestId
      {
        Value = invt.GuestId,
        VeaEventEventId = invt.EventId,
      });
    context.GuestIds.AddRange(invitations);
  }
    
  public record TmpParticipants(string GuestId, string EventId);

    private const string ParticipantsAsJson = """
                                              [
                                                {
