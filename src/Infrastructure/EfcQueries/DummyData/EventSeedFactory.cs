...
public class EventSeedFactory
{
  public static List<Event> CreateEvents()
  {
    ...
    {
      Converters = { new TmpEventConverter() }
    };

    List<TmpEvent> eventTmps = JsonSerializer.Deserialize<List<TmpEvent>>(EventsAsJson, options)!;
    ...
    public record TmpEvent(string Id, string Title, string Description, int Status, int Visibility, string StartTime, string EndTime, int MaxGuestNumber);
